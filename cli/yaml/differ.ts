...
import { YamlOntology, YamlEntityType, YamlRelationType, convertToJsonSchema } from './parser';
...
function compareSchemas(current: any, desired: any): any[] {
  ...
  if (desired.properties) {
    for (const [field, schema] of Object.entries(desired.properties)) {
      ...
      } else if (JSON.stringify(current.properties[field]) !== JSON.stringify(schema)) {
        changes.push({
          field,
          action: 'modify',
  ...
  if (JSON.stringify(currentRequired.sort()) !== JSON.stringify(desiredRequired.sort())) {
    changes.push({
      field: 'required',
      action: 'modify',
...
export async function computeDiff(
  ...
  if (yamlOntology.types) {
    for (const [typeName, typeDef] of Object.entries(yamlOntology.types)) {
      const namespace = typeDef.namespace || yamlOntology.namespace || 'default';
  ...
  if (yamlOntology.relations) {
    for (const relationDef of yamlOntology.relations) {
      const namespace = relationDef.namespace || yamlOntology.namespace || 'default';
      ...
      if (!currentRelation) {
        ...
        if (currentRelation.cardinality !== (relationDef.cardinality || 'n..n')) {
          changes.push(`cardinality: ${currentRelation.cardinality} → ${relationDef.cardinality || 'n..n'}`);
        }
...
export function formatDiff(diff: OntologyDiff): string {
  ...
  for (const typeDiff of diff.entityTypes) {
    ...
    if (typeDiff.action === 'create') {
      lines.push(`${colors.green}+ Create EntityType "${typeDiff.name}" in namespace "${typeDiff.namespace}"${colors.reset}`);
      const fields = Object.keys(typeDiff.desired?.properties || {});
      if (fields.length > 0) {
        lines.push(`  ${colors.dim}Fields: ${fields.join(', ')}${colors.reset}`);
      }
    } else if (typeDiff.action === 'update') {
      lines.push(`${colors.yellow}~ Update EntityType "${typeDiff.name}" in namespace "${typeDiff.namespace}"${colors.reset}`);
      if (typeDiff.changes) {
        for (const change of typeDiff.changes) {
          if (change.action === 'add') {
            lines.push(`  ${colors.green}+ Add field "${change.field}"${colors.reset}`);
          } else if (change.action === 'modify') {
            lines.push(`  ${colors.yellow}~ Modify field "${change.field}"${colors.reset}`);
          } else if (change.action === 'remove') {
            lines.push(`  ${colors.red}- Remove field "${change.field}"${colors.reset}`);
          }
        }
      }
    } else if (typeDiff.action === 'delete') {
      lines.push(`${colors.red}- Delete EntityType "${typeDiff.name}" in namespace "${typeDiff.namespace}"${colors.reset}`);
    }
  }
  ...
  for (const relationDiff of diff.relationTypes) {
    ...
    if (relationDiff.action === 'create') {
      lines.push(`${colors.green}+ Create RelationType "${relationDiff.name}" in namespace "${relationDiff.namespace}"${colors.reset}`);
      if (relationDiff.desired) {
        lines.push(`  ${colors.dim}${relationDiff.desired.from} → ${relationDiff.desired.to} (${relationDiff.desired.cardinality || 'n..n'})${colors.reset}`);
      }
    } else if (relationDiff.action === 'update') {
      lines.push(`${colors.yellow}~ Update RelationType "${relationDiff.name}" in namespace "${relationDiff.namespace}"${colors.reset}`);
      ...
      lines.push(`${colors.red}- Delete RelationType "${relationDiff.name}" in namespace "${relationDiff.namespace}"${colors.reset}`);
    }
  }
  ...
  if (diff.summary.typesToCreate > 0) {
    lines.push(`  ${colors.green}+ ${diff.summary.typesToCreate} entity types to create${colors.reset}`);
  }
  if (diff.summary.typesToUpdate > 0) {
    lines.push(`  ${colors.yellow}~ ${diff.summary.typesToUpdate} entity types to update${colors.reset}`);
  }
  if (diff.summary.typesToDelete > 0) {
    lines.push(`  ${colors.red}- ${diff.summary.typesToDelete} entity types to delete${colors.reset}`);
  }
  if (diff.summary.relationsToCreate > 0) {
    lines.push(`  ${colors.green}+ ${diff.summary.relationsToCreate} relation types to create${colors.reset}`);
  }
  if (diff.summary.relationsToUpdate > 0) {
    lines.push(`  ${colors.yellow}~ ${diff.summary.relationsToUpdate} relation types to update${colors.reset}`);
  }
  if (diff.summary.relationsToDelete > 0) {
    lines.push(`  ${colors.red}- ${diff.summary.relationsToDelete} relation types to delete${colors.reset}`);
  }
  
  if (diff.summary.totalChanges === 0) {
    lines.push(`  ${colors.dim}No changes detected${colors.reset}`);
  } else {
    lines.push(`  ${colors.cyan}Total: ${diff.summary.totalChanges} changes${colors.reset}`);
  }
